# Aligned with plumber-pdf-backend / hvac-pdf-backend: Chrome in-image + puppeteer-core (no Render PUPPETEER_* env required).
...
# Ensure CID_HomeBase exists in cloud builds (Render Docker does not fetch git submodules by default).
# Bump HOMEBASE_CACHE_BUST when you need a fresh HomeBase clone without a code change.
ARG HOMEBASE_CACHE_BUST=1
